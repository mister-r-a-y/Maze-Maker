interface MazeManagerType {
  grid: number[][]
  DIRECTION: Record<string, number>
  initialize: (width:number, height:number) => void
  opposite: (direction: number) => number
  path: (fromX: number, fromY: number, toX: number, toY: number, direction: number) => void
...
export var MazeManager: MazeManagerType = {
  ...
  initialize: function(width: number, height: number) {
    MazeManager.grid = [];
